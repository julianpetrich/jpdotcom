...
def test_posts_model():
    post1 = Post(title="title_1", description_long="description", slug='post_1')
    post1.save()
    post2 = Post(title="title_2", description_long="description", cover="test.png", slug='post_2')
    post2.save()
    assert Post.objects.all().count() == 2
    assert post1.title == "title_1"
    assert post1.slug == "post_1"
